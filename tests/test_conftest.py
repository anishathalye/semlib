...
@pytest.mark.asyncio
async def test_exact_match_returns_expected_response(
...
@pytest.mark.asyncio
async def test_no_match_raises_error(
    llm_mocker: Callable[[dict[str, str]], LLMMocker],
) -> None:
    ...
    with (
        mocker.patch_llm(),
        pytest.raises(
            ValueError, match="no mock response found for prompt: Goodbye world"
        ),
    ):
...
@pytest.mark.asyncio
async def test_partial_match_does_not_work(
    llm_mocker: Callable[[dict[str, str]], LLMMocker],
) -> None:
    ...
    with (
        mocker.patch_llm(),
        pytest.raises(
            ValueError, match="no mock response found for prompt: Hello world"
        ),
    ):
        await litellm.acompletion(
            model="gpt-3.5-turbo", messages=[{"role": "user", "content": "Hello world"}]
